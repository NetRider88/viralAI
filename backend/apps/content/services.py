...
Angle: {angle}"""
        ...
        # Add custom instructions at the TOP for maximum visibility
        if custom_prompt:
            logger.info(f"Custom instructions added: {custom_prompt[:50]}...")
            prompt += f"""

⚠️ CRITICAL BRAND REQUIREMENTS (MUST FOLLOW):
{custom_prompt}

YOU MUST INCORPORATE THE ABOVE REQUIREMENTS INTO YOUR RESPONSE. THIS IS MANDATORY AND NON-NEGOTIABLE.

"""
        
        custom_prompt_text = custom_prompt if custom_prompt else "No specific brand guidelines"
        prompt += f"""Platform Guidelines:
- Max caption length: {guidelines.get('caption_max_length', 2000)} characters
- Optimal hashtags: {guidelines.get('optimal_hashtags', 10)}

Requirements:
1. MANDATORY: You MUST follow these brand guidelines: {custom_prompt_text}
2. Write an engaging caption that hooks viewers in the first line
3. Include a clear call-to-action that incorporates the brand guidelines from requirement #1
